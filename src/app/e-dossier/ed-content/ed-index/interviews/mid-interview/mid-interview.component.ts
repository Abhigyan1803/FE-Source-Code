...
@Component({
  selector: 'ms-mid-interview',
  templateUrl: './mid-interview.component.html',
  styleUrls: ['./mid-interview.component.scss']
})
export class MidInterviewComponent implements OnInit {
  ...
  getEdossierInterview(){
    this.EDossierService.getMidInterviewDetails(this.Id).subscribe(
      res => {
        if (res && res.object) {
...
addInitialsubmitI(formVal,cdrType) {
  ...
  this.EDossierService.addMidInterviewDetails(formVal).subscribe(
    res => {
      console.log(res);
      if (res.status == 'OK') {
...
updateInterview(formVal,cdrType) {
  ...
  this.EDossierService.updateMidInterviewDetails(formVal).subscribe(
    res => {
      console.log(res);
