...
@Component({
  ...
  templateUrl: './communication-sec.component.html',
  styleUrls: ['./communication-sec.component.scss']
})
export class CommunicationSecComponent implements OnInit {
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  awardeesList:any[]=[];
  dataSource: any;
  displayedColumns: string[] = ['number', 'name','description','document','status','action'];  
  Communicationsec: any[] = [];
  ...
  getItCommunicationSec(){
    this.spinner.show();
  this.adminservice.getCommunicationSec(2).subscribe(res =>{  
    console.log(res);
    
    if(res.status=="OK"){
      this.Communicationsec = res.object ;
...
console.log(res,"=================");
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Communicationsec.slice();
    if (!sort.active || sort.direction === '') {
      this.Communicationsec = datalist;
      return;
    }
    this.Communicationsec = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Communicationsec);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
  ...
  changeCommunicationSecStatus(e, d) {
    this.spinner.show();
    if (e.checked) {
      this.adminservice.changeCommunicationSecStatus(d.id, 1).subscribe(
        res => {
          console.log(res);
      ...
      this.adminservice.changeCommunicationSecStatus(d.id, 0).subscribe(
        res => {
          console.log(res);
