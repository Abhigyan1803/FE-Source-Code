...
@Component({
  selector: 'ms-add-biometric-rfid',
  templateUrl: './add-biometric-rfid.component.html',
  styleUrls: ['./add-biometric-rfid.component.scss']
})
export class AddBiometricRfidComponent implements OnInit {

  unSelectedFile: any;
  pageTitle = "Add Biometric";
  battalions:any[]=[];
  descLength:number = 0;
  id: string = '';
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-biometric')) {
      this.pageTitle = 'Add Biometric'
    }
    else if (this.router.url.includes('view-biometric')) {
      this.spinner.show()
      this.pageTitle = 'View Biometric'
      this.id = this.route.snapshot.queryParamMap.get('id');
      this.adminservice.getBIOMETRICByID(this.id).subscribe(
        res => {
          console.log(res);
  ...
  goBack() {
    this.router.navigate(['main/admin/GS-Branch/security/access-control/biometric-rfid']);
  }
  ...
  addBiometric() {
    if (this.addBiometricForm.invalid|| !this.isDoc) {
      ...
      this.spinner.show();
      this.adminservice.addBIOMETRIC(this.addBiometricForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
...
updateBiometric() {
  ...
  //   this.adminservice.updateBIOMETRIC(this.addBiometricForm.value,this.id).subscribe(
  //     res => {
  //       // console.log(res);
  //       this.apiRes(res);
  //     },
...
apiRes(res) {
  if (res.status == 'OK') {
    ...
    this.router.navigate(['main/GS-Branch/security/access-control/biometric-rfid'])
