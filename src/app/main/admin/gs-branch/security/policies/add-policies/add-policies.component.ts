...
export class AddPoliciesComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-policies')) {
      this.pageTitle = 'Add Policies'
    }
    else if (this.router.url.includes('view-policies')) {
      this.spinner.show()
      this.pageTitle = 'View Policies'
      this.id = this.route.snapshot.queryParamMap.get('id');
      this.adminservice.getPoliciesByID(this.id).subscribe(
        res => {
          console.log(res);
  ...
  goBack() {
    this.router.navigate(['main/admin/GS-Branch/security/policies']);
  }

  addPolicies() {
    if (this.addPoliciesForm.invalid|| !this.isDoc) {
      this.isError = true;
      for (const key of Object.keys(this.addPoliciesForm.controls)) {
        if (this.addPoliciesForm.controls[key].invalid) {
      ...
      this.spinner.show();
      this.adminservice.addPolicies(this.addPoliciesForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
...
updatePolicies() {
  ...
  //   this.adminservice.updatePolicies(this.addPoliciesForm.value,this.id).subscribe(
  //     res => {
  //       // console.log(res);
  //       this.apiRes(res);
  //     },
...
apiRes(res) {
  if (res.status == 'OK') {
    ...
    this.router.navigate(['main/GS-Branch/security/policies'])
