...
export class AddReportsComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-return')) {
      ...
      this.adminservice.getIntelligenceByIDReport(this.id).subscribe(
        res => {
          console.log(res);
  ...
  goBack() {
    this.router.navigate(['main/admin/GS-Branch/security/intelligence/intelligence-reports']);
  }
  ...
  addReturn() {
    if (this.addPoliciesForm.invalid) {
      ...
      this.spinner.show();
      this.adminservice.addIntelligenceReport(this.addPoliciesForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
...
updateReturn() {
  ...
  if (this.addPoliciesForm.valid) {
    this.adminservice.updateIntelligenceReport(this.addPoliciesForm.value,this.id).subscribe(
      res => {
        // console.log(res);
        this.apiRes(res);
      },
