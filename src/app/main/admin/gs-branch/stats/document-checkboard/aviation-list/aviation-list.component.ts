...
@Component({
  ...
  templateUrl: './aviation-list.component.html',
  styleUrls: ['./aviation-list.component.scss']
})
export class AviationListComponent implements OnInit {
  ...
  getAviation(){
    this.spinner.show();
  this.adminservice.getAviation(2).subscribe(res =>{  
    console.log(res);
    
    if(res.status=="OK"){
      this.Aviation=res.object ;
...
console.log(res,"=================");
  ...
  sortData(sort: Sort) {
    const datalist = this.Aviation.slice();
    if (!sort.active || sort.direction === '') {
      this.Aviation = datalist;
      return;
    }
    this.Aviation = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Aviation);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
  ...
  openDoc(e){
    ...
    changeAviationStatus(e, d) {
      this.spinner.show();
      if (e.checked) {
        this.adminservice.changeAviationStatus(d.id, 1).subscribe(
          res => {
            console.log(res);
        ...
        this.adminservice.changeAviationStatus(d.id, 0).subscribe(
          res => {
            console.log(res);
