...
@Component({
  selector: 'ms-para-list',
  templateUrl: './para-list.component.html',
  styleUrls: ['./para-list.component.scss']
})
...
getPara(){
  this.spinner.show();
this.adminservice.getPara(2).subscribe(res =>{  
  console.log(res);
...
addPara() {
  this.router.navigate(['/main/admin/GS-Branch/stats/document-checkboard/para-list/add-ParaList']);
}
...
viewPara(element) {
  this.router.navigate(['main/admin/GS-Branch/stats/document-checkboard/para-list/view-ParaList'],{queryParams:{id:element.id}})  

  if(this.router.url.includes('main/GS-Branch'))
  this.router.navigate(['main/gs-branch/stats/document-checkboard/para-list/view-ParaList'],{queryParams:{id:element.id}})
  ...
  this.dialog.open(DialogComponent,
    {
      ...
      data: {
        type: 'document',title:"ParaList Document", url: e.doc
      }
    }
    )
  }

  changeParaStatus(e, d) {
    this.spinner.show();
    if (e.checked) {
      this.adminservice.changeParaStatus(d.id, 1).subscribe(
        res => {
          console.log(res);
