...
export class MiscComponent implements OnInit {
  ...
  getMisc(){
    this.spinner.show();
  this.adminservice.getMisc(2).subscribe(res =>{  
    console.log(res);
    
    if(res.status=="OK"){
      this.Misc=res.object ;
...
console.log(res,"=================");
  ...
  sortData(sort: Sort) {
    const datalist = this.Misc.slice();
    if (!sort.active || sort.direction === '') {
      this.Misc = datalist;
      return;
    }
    this.Misc = datalist.sort((a: any, b: any) => {
  ...
  openDoc(e){
    ...
    changeMiscStatus(e, d) {
      this.spinner.show();
      if (e.checked) {
        this.adminservice.changeMiscStatus(d.id, 1).subscribe(
          res => {
            console.log(res);
        ...
        this.adminservice.changeMiscStatus(d.id, 0).subscribe(
          res => {
            console.log(res);
