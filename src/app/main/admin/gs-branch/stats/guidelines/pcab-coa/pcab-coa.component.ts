...
export class PcabCoaComponent implements OnInit {
  ...
  getPCABAndCOA(){
    this.spinner.show();
  this.adminservice.getPCABAndCOA(2).subscribe(res =>{  
    console.log(res);
...
console.log(res,"=================");
  ...
  addPCAB() {
    this.router.navigate(['/main/admin/GS-Branch/stats/guidelines/pcab-coa/add-pcab-coa/']);
  }
  ...
  openDoc(e){
    ...
    changePCABAndCOABStatus(e, d) {
      this.spinner.show();
      if (e.checked) {
        this.adminservice.changePCABAndCOABStatus(d.id, 1).subscribe(
          res => {
            console.log(res);
        ...
        this.adminservice.changePCABAndCOABStatus(d.id, 0).subscribe(
          res => {
            console.log(res);
