...
export class MatrixComponent implements OnInit {
  ...
  getMatrix(){
    this.spinner.show();
  this.adminservice.getMatrix(2).subscribe(res =>{  
    console.log(res);
    
    if(res.status=="OK"){
      this.Matrix=res.object ;
...
console.log(res,"=================");
  ...
  sortData(sort: Sort) {
    const datalist = this.Matrix.slice();
    if (!sort.active || sort.direction === '') {
      this.Matrix = datalist;
      return;
    }
    this.Matrix = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Matrix);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
  ...
  openDoc(e){
    ...
    changeMatrixStatus(e, d) {
      this.spinner.show();
      if (e.checked) {
        this.adminservice.changeMatrixStatus(d.id, 1).subscribe(
          res => {
            console.log(res);
        ...
        this.adminservice.changeMatrixStatus(d.id, 0).subscribe(
          res => {
            console.log(res);
