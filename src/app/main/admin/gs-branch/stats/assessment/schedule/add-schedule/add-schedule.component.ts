...
export class AddScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-schedule')) {
      ...
      this.adminservice.getScheduleByID(this.id).subscribe(
        res => {
          console.log(res);
  ...
  goBack() {
    this.router.navigate(['main/admin/GS-Branch/stats/assessment/schedule']);
  }
  ...
  addSchedule() {
    if (this.addScheduleForm.invalid || !this.isDoc) {
      ...
      this.spinner.show();
      this.adminservice.addSchedule(this.addScheduleForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
...
updateSchedule() {
  ...
  if (this.addScheduleForm.valid) {
    this.adminservice.updateSchedule(this.addScheduleForm.value,this.id).subscribe(
      res => {
        // console.log(res);
        this.apiRes(res);
      },
...
apiRes(res) {
  if (res.status == 'OK') {
    ...
    this.router.navigate(['main/GS-Branch/stats/assessment/schedule'])
