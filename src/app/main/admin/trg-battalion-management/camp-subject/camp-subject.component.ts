...
@Component({
  ...
  templateUrl: './camp-subject.component.html',
  styleUrls: ['./camp-subject.component.scss']
})
export class CampSubjectComponent implements OnInit {

  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;

  dataSource: any;
  displayedColumns: string[] = ['number', 'subjectName','totalMarks','status','action'];  
  POC: any[] = [];
  
  
  constructor(private router: Router, private adminservice: TrgBattalionService, private spinner:NgxSpinnerService,
  ...
  getSubjectCamp(){
    this.spinner.show();
  this.adminservice.getSubjectCamp(2).subscribe(res =>{  
    console.log(res);
  ...
  sortData(sort: Sort) {
    ...
    changeCampSubjStatus(e, d) {
      this.spinner.show();
      if (e.checked) {
        this.adminservice.getCAMPSUBJCHANGESTATUS(d.id, 1).subscribe(
          res => {
            console.log(res);
            if (res.status == 'OK') {
              this.adminservice.openSnackbar(res.message)
        ...
        this.adminservice.getCAMPSUBJCHANGESTATUS(d.id, 0).subscribe(
          res => {
            console.log(res);
