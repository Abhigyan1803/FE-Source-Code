...
export class AddGcMsgComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private service: AdminService, private router: Router, private spinner: NgxSpinnerService, private dialog: MatDialog,
    private route: ActivatedRoute, private snackbar: MatSnackBar, private cdref: ChangeDetectorRef, @Inject(LOCALE_ID) localID: string, public sharedService: SharedService) {
...
console.log(this.addProgramForm,"addProgramForm===");
 ...
 let fileSizeMatch = this.sharedService.checkFileSize(file);
    ...
    if (!fileSizeMatch) {
      ...
      this.service.openSnackbar(`Document Should Be Maximum ${this.sharedService.fileSize} MB in Size`)
    } else {
      this.docUrl = ''
      this.addProgramForm.patchValue({
        doc: file
