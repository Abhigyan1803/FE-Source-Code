...
export class AddSportsComponent implements OnInit {
  ...
  constructor(
    ...
    private activeRoute: ActivatedRoute,private AcademicDeptService:AcademicDeptService) {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.includes('add-sports')) {
      this.spinner.show();
      ...
      var termSession = getForm.termSession
      this.AcademicDeptService.getSports(serviceId,termId,termSession).subscribe(
        res => {
  ...
  submit() {
    ...
    this.totalMarks = Object.assign({}, this.drillForm.value, { obtainedMarks: this.totalmarkst, totalMarks: this.getTotal(), status: 1, termId: 1 });
    ...
    this.AcademicDeptService.addSports(formdata).subscribe(
      res => {
        if (res.message == 'OK') {
  ...
  confirm() {
    ...
    //   status: 1,
    // }
    this.totalMarks1 = Object.assign({}, this.drillForm.value, { obtainedMarks: this.totalmarkst, totalMarks: this.updatetotalmarks, status: 1, termId: 1, id: this.drilleditid });
    ...
    this.AcademicDeptService.updateSports(formdata).subscribe(
      res => {
        if (res.message == 'OK') {
