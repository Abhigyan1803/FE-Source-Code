...
export class NominalComponent implements OnInit {
  ...
  dataSource: any;
  displayedColumns: string[] = ['number', 'name', 'createdAt', 'document', 'status', 'delete', 'action'];
  PCHT: any[] = [];
  ...
  ngAfterViewInit() {
    this.getAchievements('ACDCLUBS', 'GC Nominal Role')
  }

  getAchievements(ACDCLUBS, NOMINAL) {
    this.spinner.show();
    this.adminservice.getClubs(ACDCLUBS, NOMINAL, 2).subscribe(res => {
  ...
  addDay() {
    if (this.router.url.includes('main/academic-depart'))
      this.router.navigate(['main/academic-depart/clubs/nominal/add-nominal']);

    if (this.router.url.includes('main/trg-team/adventure-cell/clubs/nominal'))
      this.router.navigate(['main/trg-team/adventure-cell/clubs/nominal/add-nominal']);
  }
  ...
  viewGC(element) {
    this.router.navigate(['main/academic-depart/clubs/nominal/view-nominal'], { queryParams: { id: element.id } })
  }
  ...
  delete(element) {
    this.adminservice.updateClubsStatus(element.id, 3).subscribe(
      res => {
        console.log(res);
        if (res.status == 'OK') {
          ...
          this.cdref.detectChanges();
          this.spinner.hide();
          this.router.navigate(['main/academic-depart/clubs/nominal']);
        } else {
          this.spinner.hide();
          this.adminservice.openSnackbar(res.message)
        }
