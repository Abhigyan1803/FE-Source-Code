...
export class AddNominalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-nominal')) {
      this.pageTitle = 'Add Nominal'
    }
    else if (this.router.url.includes('view-nominal')) {
      this.spinner.show()
      this.pageTitle = 'View Nominal'
