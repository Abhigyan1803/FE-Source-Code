...
export class SopsComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.getAchievements('ACDCLUBS', 'SOPs')
  }

  getAchievements(ACDCLUBS, SOPs) {
    this.spinner.show();
    this.adminservice.getClubs(ACDCLUBS, SOPs, 2).subscribe(res => {
      console.log(res);
  ...
  addDay() {
    ...
    if (this.router.url.includes('/main/academic-depart'))
      this.router.navigate(['main/academic-depart/clubs/sops/add-sop']);

    if (this.router.url.includes('main/trg-team/adventure-cell/clubs/sops'))
      this.router.navigate(['main/trg-team/adventure-cell/clubs/sops/add-sop']);
  ...
  viewGC(element) {
    this.router.navigate(['main/academic-depart/clubs/sops/view-sop'], { queryParams: { id: element.id } })
  }
    ...
    this.adminservice.updateClubsStatus(element.id, 3).subscribe(
      res => {
        ...
        if (res.status == 'OK') {
          ...
          this.cdref.detectChanges();
          this.spinner.hide();
          this.router.navigate(['main/academic-depart/clubs/sops']);
        } else {
          this.spinner.hide();
          this.adminservice.openSnackbar(res.message)
        }
