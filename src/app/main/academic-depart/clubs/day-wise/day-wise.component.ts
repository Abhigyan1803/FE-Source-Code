...
export class DayWiseComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.getAchievements('ACDCLUBS', 'Daywise Programme')
  }

  getAchievements(ACDCLUBS, DAYWISE) {
    this.spinner.show();
    this.adminservice.getClubs(ACDCLUBS, DAYWISE, 2).subscribe(res => {
      console.log(res);
  ...
  addDay() {
    ...
    this.router.navigate(['/main/academic-depart/clubs/day-wise/add-day']);
    
    if(this.router.url.includes('/main/trg-team/adventure-cell/clubs/day-wise'))
    this.router.navigate(['/main/trg-team/adventure-cell/clubs/day-wise/add-day']);
  ...
  viewGC(element) {
    this.router.navigate(['main/academic-depart/clubs/day-wise/view-day'], { queryParams: { id: element.id } })
  }
