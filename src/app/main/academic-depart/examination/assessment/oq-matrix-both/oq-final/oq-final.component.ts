...
export class OqFinalComponent implements OnInit {
  ...
  allData() {
    this.academicservice.getOQ_Final_matrix_list(this.termId, this.currentPage, this.pageSize).subscribe(res => {
      console.log(res);
  ...
  getSpecialAllList() {
    this.academicservice.getOQ_Final_All_ListByBCNameComp(this.termId, this.currentPage, this.pageSize, this.battalionName).subscribe(res => {
      console.log(res);
      if (res.message == 'OK') {
        this.resultsLength = res.object.totalRecords;
        this.oqCadetList = res.object.oqMatrixFilterPayload;
  ...
  onChange(e: any, value, totalMarks, sub_id, mainIndex, subIndex) {
    ...
    this.oqCadetList[mainIndex].academicOqMatrixResult.academicOqMatrixSubjectResult[subIndex].obtainedMarks = '';
      ...
      this.oqCadetList[mainIndex].academicOqMatrixResult.termType = 'FINAL-TERM';
    ...
    if (this.serviceid == null || this.serviceid == undefined || this.serviceid == '') {
      ...
      merge(/* this.sort.sortChange,  */this.paginator.page)
        .pipe(
          ...
          switchMap(() => {
            this.spinner.show()
            return this.academicservice.getOQ_Final_Matrix_search(this.termId, this.serviceid, this.paginator.pageIndex, this.paginator.pageSize)
          }), map(data => {
  ...
  getOQAllCadetlist(event?: PageEvent) {
    if (this.battalionId == 1 || this.battalionId == 2 || this.battalionId == 3 || this.battalionId == 4) {
      this.academicservice.getOQ_Final_All_ListByBCName(this.termId, this.currentPage, this.pageSize, this.battalionName, this.companyName).subscribe(res => {
        console.log(res);
        if (res.message == 'Record not found') {
          this.spinner.hide();
          this.academicservice.openSnackbar(res.message);
