...
@Component({
  selector: 'ms-intellectual-skills',
  templateUrl: './intellectual-skills.component.html',
  styleUrls: ['./intellectual-skills.component.scss']
})
export class IntellectualSkillsComponent implements OnInit {
  ...
  onChange(e: any, value, totalMarks, sub_id, mainIndex, subIndex) {
    ...
    this.intellectualCadetList[mainIndex].intellectualSkillsResult.intellectualSkillsSubResult[subIndex].finalObtainedMarks = '';
      return false;
    }
    else {
      ...
      if (total) {
        ...
        this.intellectualCadetList[mainIndex].intellectualSkillsResult.finalObtainedMarks = parseInt(total) + (value == '' ? 0 : parseInt(value));
      } else {
        (<HTMLInputElement>document.getElementById(mainIndex)).value = value;
        this.intellectualCadetList[mainIndex].intellectualSkillsResult.finalObtainedMarks = (value == '' ? 0 : parseInt(value));
      }
      this.intellectualCadetList[mainIndex].intellectualSkillsResult.intellectualSkillsSubResult[subIndex].finalObtainedMarks = value;
    }
  ...
  onFocusEvent(value, mainIndex) {
    ...
    if (total) {
      ...
      this.intellectualCadetList[mainIndex].intellectualSkillsResult.finalObtainedMarks = temp == 0?null:temp ;
    } else {
      (<HTMLInputElement>document.getElementById(mainIndex)).value = value;
      this.intellectualCadetList[mainIndex].intellectualSkillsResult.finalObtainedMarks = value;
    }
    ...
    this.academicservice.getIntellectual_Subject(1, this.termId).subscribe(res => {
      if (res.status == "OK") {
        ...
        for (let i = 0; i < this.subjectSize; i += 1) {
          this.totalSubjectmarks += res.object[i].finalTotalMarks;
          //res.object.totalMarks[i] = number + 1;
