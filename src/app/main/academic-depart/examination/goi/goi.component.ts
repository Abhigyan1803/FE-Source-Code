...
@Component({
  selector: 'ms-goi',
  templateUrl: './goi.component.html',
  styleUrls: ['./goi.component.scss']
})
export class GoiComponent implements OnInit {
  ...
  constructor(private router: Router, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    getGoi(GOI){
      this.spinner.show();
    this.adminservice.getGOILIST(GOI,2).subscribe(res =>{  
      console.log(res);
    ...
    addGC() {
      this.router.navigate(['main/academic-depart/examination/goi/add-goi']);
    }
    ...
    viewGC(element) {
      this.router.navigate(['main/academic-depart/examination/goi/view-goi'],{queryParams:{id:element.id}})  
    }
