...
export class QuizComponent implements OnInit {
  ...
  constructor(private router: Router, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    ngAfterViewInit() {
      this.getAchievements('GENERAL','CURRICULAR','QUIZ')
    } 
  
    getAchievements(GENERAL,CURRICULAR,QUIZ){
      this.spinner.show();
    this.adminservice.getSOPs(GENERAL,CURRICULAR,QUIZ,2).subscribe(res =>{  
      console.log(res);
    ...
    addDay() {
      this.router.navigate(['main/academic-depart/general/curricular/quiz/add-quiz']);
    }
    ...
    viewGC(element) {
      this.router.navigate(['main/academic-depart/general/curricular/quiz/view-quiz'],{queryParams:{id:element.id}})  
    }
