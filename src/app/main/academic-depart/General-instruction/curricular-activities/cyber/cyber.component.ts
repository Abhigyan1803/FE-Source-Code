...
export class CyberComponent implements OnInit {
  ...
  constructor(private router: Router, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    ngAfterViewInit() {
      this.getAchievements('GENERAL','CURRICULAR','CYBER')
    }
  
    getAchievements(GENERAL,CURRICULAR,CYBER){
      this.spinner.show();
    this.adminservice.getSOPs(GENERAL,CURRICULAR,CYBER,2).subscribe(res =>{  
      console.log(res);
    ...
    addDay() {
      this.router.navigate(['main/academic-depart/general/curricular/cyber/add-cyber']);
    }
    ...
    viewGC(element) {
      this.router.navigate(['main/academic-depart/general/curricular/cyber/view-cyber'],{queryParams:{id:element.id}})  
    }
      ...
      this.dialog.open(DialogComponent,
        {
          ...
          data: {
            type: 'document',title:"View Document", url: e.doc
          }
        }
        )
      }
