...
@Component({
  selector: 'ms-english-deb',
  templateUrl: './english-deb.component.html',
  styleUrls: ['./english-deb.component.scss']
})
export class EnglishDebComponent implements OnInit {
  ...
  constructor(private router: Router, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    ngAfterViewInit() {
      this.getAchievements('GENERAL','CURRICULAR','ENGLISHDEB',)
    }
  
    getAchievements(GENERAL,CURRICULAR,ENGLISHDEB){
      this.spinner.show();
    this.adminservice.getSOPs(GENERAL,CURRICULAR,ENGLISHDEB,2).subscribe(res =>{  
      console.log(res);
    ...
    addDay() {
      this.router.navigate(['main/academic-depart/general/curricular/english/add-english']);
    }
    ...
    viewGC(element) {
      this.router.navigate(['main/academic-depart/general/curricular/english/view-english'],{queryParams:{id:element.id}})  
    }
