...
@Component({
  selector: 'ms-add-english-deb',
  templateUrl: './add-english-deb.component.html',
  styleUrls: ['./add-english-deb.component.scss']
})
export class AddEnglishDebComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-english')) {
      this.pageTitle = 'Add English'
    }
    else if (this.router.url.includes('view-english')) {
      this.spinner.show()
      this.pageTitle = 'View English'
  ...
  goBack() {
    this.router.navigate(['main/academic-depart/general/curricular/english']);
  }
  ...
  addCyber() {
    if (this.addPCHTForm.invalid || !this.isDoc) {
      ...
      this.spinner.show();
      this.adminservice.addSOPs(this.addPCHTForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
...
onSelectDoc(e) {
  ...
  //change file from 50 mb to 200 mb
