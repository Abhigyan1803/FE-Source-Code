...
@Component({
  selector: 'ms-add-ppt',
  templateUrl: './add-ppt.component.html',
  styleUrls: ['./add-ppt.component.scss']
})
export class AddPptComponent implements OnInit {
  unSelectedFile: any;
  pageTitle = "Add Ppt";
  battalions:any[]=[];
  descLength:number = 0;
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-ppt')) {
      this.pageTitle = 'Add Ppt'
    }
    else if (this.router.url.includes('view-ppt')) {
      this.spinner.show()
      this.pageTitle = 'View Ppt'
  ...
  goBack() {
    this.router.navigate(['main/academic-depart/general/curricular/ppt']);
  }
