...
@Component({
  selector: 'ms-add-hindi-deb',
  templateUrl: './add-hindi-deb.component.html',
  styleUrls: ['./add-hindi-deb.component.scss']
})
export class AddHindiDebComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-hindideb')) {
      this.pageTitle = 'Add Hindi'
    }
    else if (this.router.url.includes('view-hindideb')) {
      this.spinner.show()
      this.pageTitle = 'View Hindi'
  ...
  goBack() {
    this.router.navigate(['main/academic-depart/general/curricular/hindi']);
  }
