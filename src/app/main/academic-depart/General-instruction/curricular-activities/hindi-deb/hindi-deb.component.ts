...
@Component({
  selector: 'ms-hindi-deb',
  templateUrl: './hindi-deb.component.html',
  styleUrls: ['./hindi-deb.component.scss']
})
export class HindiDebComponent implements OnInit {
  ...
  constructor(private router: Router, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    ngAfterViewInit() {
      this.getAchievements('GENERAL','CURRICULAR','HINDIDEB',)
    }
  
    getAchievements(GENERAL,CURRICULAR,HINDIDEB){
      this.spinner.show();
    this.adminservice.getSOPs(GENERAL,CURRICULAR,HINDIDEB,2).subscribe(res =>{  
      console.log(res);
    ...
    addDay() {
      this.router.navigate(['main/academic-depart/general/curricular/hindi/add-hindideb']);
    }
    ...
    viewGC(element) {
      this.router.navigate(['main/academic-depart/general/curricular/hindi/view-hindideb'],{queryParams:{id:element.id}})  
    }
