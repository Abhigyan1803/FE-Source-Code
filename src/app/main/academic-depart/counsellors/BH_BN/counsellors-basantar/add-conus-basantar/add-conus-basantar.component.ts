...
@Component({
  ...
  templateUrl: './add-conus-basantar.component.html',
  styleUrls: ['./add-conus-basantar.component.scss']
})
export class AddConusBasantarComponent implements OnInit {

  pageTitle = "Add BASANTAR";
  id: string = '';
  isAdmin:boolean = false;
  addbasantarForm: FormGroup = new FormGroup({});
  isError: boolean;
    ...
    if (this.router.url.includes('add-basantar')) {
      ...
      this.adminservice.getCounsellorById(this.id).subscribe(
        res => {
          ...
          if (res.status == 'OK') {
            this.addbasantarForm.patchValue({
  ...
  addBASANTAR() {
    if (this.addbasantarForm.invalid) {
      ...
      this.spinner.show();
      this.adminservice.addCounsellor(this.addbasantarForm.value).subscribe(
        res => {
          console.log(res);
          if (res.status == 'OK') {
            this.adminservice.openSnackbar(res.message)
  ...
  if (this.addbasantarForm.invalid) {
    ...
    this.adminservice.updateCounsellor(this.addbasantarForm.value).subscribe(
      res => {
        console.log(res);
        if (res.status == 'OK') {
          this.adminservice.openSnackbar(res.message)
