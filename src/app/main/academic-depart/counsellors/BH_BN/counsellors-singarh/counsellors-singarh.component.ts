...
@Component({
  ...
  templateUrl: './counsellors-singarh.component.html',
  styleUrls: ['./counsellors-singarh.component.scss']
})
export class CounsellorsSingarhComponent implements OnInit {
...
console.log(res,"=================");
  ...
  sortData(sort: Sort){
    const datalist = this.Singarh.slice();
    if (!sort.active || sort.direction === '') {
      this.Singarh = datalist;
      return;
    }
    this.Singarh = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Singarh);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
