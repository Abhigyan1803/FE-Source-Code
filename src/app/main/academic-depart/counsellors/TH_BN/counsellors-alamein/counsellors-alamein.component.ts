...
@Component({
  ...
  templateUrl: './counsellors-alamein.component.html',
  styleUrls: ['./counsellors-alamein.component.scss']
})
export class CounsellorsAlameinComponent implements OnInit {
...
console.log(res,"=================");
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Alamein.slice();
    if (!sort.active || sort.direction === '') {
      this.Alamein = datalist;
      return;
    }
    this.Alamein = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Alamein);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
