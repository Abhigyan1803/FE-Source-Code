...
@Component({
  selector: 'ms-counsellors-dograi',
  templateUrl: './counsellors-dograi.component.html',
  styleUrls: ['./counsellors-dograi.component.scss']
})
export class CounsellorsDograiComponent implements OnInit {
...
console.log(res,"=================");
  ...
  addDOGRAI() {
    this.router.navigate(['/main/academic-depart/counsellors/thbn/counsellors-dograi/add-dograi']);
  }
  ...
  viewDOGRAI(element) {
    ...
    this.router.navigate(['/main/academic-depart/counsellors/thbn/counsellors-dograi/view-dograi'],{queryParams:{id:element.id}})
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Dograi.slice();
    if (!sort.active || sort.direction === '') {
      this.Dograi = datalist;
      return;
    }
    this.Dograi = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Dograi);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
