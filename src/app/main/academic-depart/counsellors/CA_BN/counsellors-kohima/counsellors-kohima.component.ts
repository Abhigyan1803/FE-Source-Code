...
@Component({
  selector: 'ms-counsellors-kohima',
  templateUrl: './counsellors-kohima.component.html',
  styleUrls: ['./counsellors-kohima.component.scss']
})
export class CounsellorsKohimaComponent implements OnInit {
...
console.log(res,"=================");
  ...
  addKOHIMA() {
    this.router.navigate(['/main/academic-depart/counsellors/cabn/counsellors-kohima/add-kohima']);
  }
  ...
  viewKOHIMA(element) {
    ...
    this.router.navigate(['/main/academic-depart/counsellors/cabn/counsellors-kohima/view-kohima'],{queryParams:{id:element.id}})
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Itsec.slice();
    if (!sort.active || sort.direction === '') {
      this.Itsec = datalist;
      return;
    }
    this.Itsec = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Itsec);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
