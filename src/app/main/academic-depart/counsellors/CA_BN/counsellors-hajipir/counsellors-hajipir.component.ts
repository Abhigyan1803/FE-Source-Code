...
@Component({
  selector: 'ms-counsellors-hajipir',
  templateUrl: './counsellors-hajipir.component.html',
  styleUrls: ['./counsellors-hajipir.component.scss']
})
export class CounsellorsHajipirComponent implements OnInit {
...
console.log(res,"=================");
  ...
  addHAJIPIR() {
    this.router.navigate(['/main/academic-depart/counsellors/cabn/counsellors-hajipir/add-hajipir']);
  }
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Hajipir.slice();
    if (!sort.active || sort.direction === '') {
      this.Hajipir = datalist;
      return;
    }
    this.Hajipir = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Hajipir);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
