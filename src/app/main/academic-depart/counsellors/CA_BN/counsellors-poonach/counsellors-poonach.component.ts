...
@Component({
  ...
  templateUrl: './counsellors-poonach.component.html',
  styleUrls: ['./counsellors-poonach.component.scss']
})
export class CounsellorsPoonachComponent implements OnInit {
...
console.log(res,"=================");
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Poonach.slice();
    if (!sort.active || sort.direction === '') {
      this.Poonach = datalist;
      return;
    }
    this.Poonach = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Poonach);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
