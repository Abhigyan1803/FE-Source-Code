...
@Component({
  ...
  templateUrl: './counsellors-naushera.component.html',
  styleUrls: ['./counsellors-naushera.component.scss']
})
export class CounsellorsNausheraComponent implements OnInit {
...
console.log(res,"=================");
  ...
  sortData(sort: Sort){
    ...
    const datalist = this.Naushera.slice();
    if (!sort.active || sort.direction === '') {
      this.Naushera = datalist;
      return;
    }
    this.Naushera = datalist.sort((a: any, b: any) => {
    ...
    this.dataSource = new MatTableDataSource(this.Naushera);
    this.dataSource.paginator = this.paginator;
    if (this.dataSource.paginator) {
      this.dataSource.paginator.firstPage();
    }
