...
export class MidTermComponent implements OnInit {
  ...
  allData(){
    this.adjutantservice.getOQ_DRILL_list(this.termId, this.currentPage, this.pageSize).subscribe(res => {
      console.log(res);
  ...
  getRunBackAllCadetlist() {
    ...
    if (this.battalionId == 1 || this.battalionId == 2 || this.battalionId == 3 || this.battalionId == 4) {
      this.adjutantservice.getOQ_All_ListByBCName( this.termId, this.currentPage, this.pageSize, this.battalionName, this.companyName).subscribe(res => {
        console.log(res);
        if (res.message == 'Record not found') {
          this.spinner.hide();
          this.adjutantservice.openSnackbar(res.message);
  ...
  confirm() {
    ...
    this.adjutantservice.updateOQDrill(formdata).subscribe(
      res => {
  ...
  edSearch(event?: PageEvent) {
    if (this.serviceid == null || this.serviceid == undefined || this.serviceid == '') {
      ...
      this.adjutantservice.getOQ_DRILL_search( this.termId, this.serviceid, this.paginator.pageIndex, this.paginator.pageSize).subscribe(res => {
        console.log(res);
        if (res.message == 'Record not found') {
          this.spinner.hide();
          this.adjutantservice.openSnackbar(res.message);
