...
@Component({
  selector: 'ms-add-drill-marks',
  templateUrl: './add-drill-marks.component.html',
  styleUrls: ['./add-drill-marks.component.scss']
})
export class AddDrillMarksComponent implements OnInit {
  ...
  genSubRec() {
    return this.fb.group({
      ...
      subjectId: [''],
      subjectName: [''],
      termId: [this.drillForm.value.termId],
      totalMarks: ['']
    })
  }
  serId
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.includes('add-drill-marks')) {
      ...
      var termId = this.drillForm.value.termId
      this.adminservice.getDrillMark(serviceId, termId).subscribe(
        res => {
          ...
          if (res.message == "update") {
            ...
            let cmrks = res.object.dRILLSubjectResult;
            ...
            this.drillForm.patchValue({
              dRILLSubjectResult: drillResult,
            })

            // console.log(this.obtainedmarks, "obtainedmarks")
            // this.dataSource = new MatTableDataSource(res.object.dRILLSubjectResult);
            this.drillForm.patchValue({
            ...
            this.drillForm.value.dRILLSubjectResult = []
            this.mnc = "add";
            // this.isShown = true
            this.Campmarks = res.object;
  ...
  changenew(e: any) {
    let frmArray = this.drillForm.get('dRILLSubjectResult') as FormArray;
    frmArray.clear();
    this.serviceId = this.drillForm.value.serviceId
    this.termId = this.drillForm.value.term
    this.spinner.show()
    this.adminservice.getSubjectMarks_List(this.serviceId, this.termId, e).subscribe(res => {
      if (res.message == "update") {
        ...
        this.obtainedmarks = res.object.dRILLSubjectResult;
        ...
        this.mnc = "add";
        this.Campmarks = res.object;
        this.drillForm.controls['subject'].setValue('');
        this.drillForm.controls['gcAppt'].setValue('');
        this.drillForm.controls['remarks'].setValue('');
        this.dataSource = new MatTableDataSource(res.object);
        this.cdref.detectChanges();
      }
    },
  ...
  submit() {
    ...
    //     status: 1,
    //   }
    //   this.drillForm.value.dRILLSubjectResult.push(marksData);
    //   indexT++;
    //})
   
    this.totalMarks = Object.assign({}, this.drillForm.value, { obtainedMarks: this.totalmarkst, totalMarks: this.getTotal(), status: 1, termId:this.drillForm.value.termId });
    ...
    for (let i = 0; i < formdata.dRILLSubjectResult.length; i++) {
      delete formdata.dRILLSubjectResult[i].subjectName
      }

    console.log(formdata, "finalresult");
    this.adminservice.addDrillMarks(formdata).subscribe(
      res => {
        if (res.message == 'OK') {
          this.adminservice.openSnackbar("Drill Marks Added Successfully");
          // this.spinner.hide()
        }
        err => {
          // this.spinner.hide()
  ...
  confirm() {
    ...
    //   status: 1,
    // }
    this.totalMarks1 = Object.assign({}, this.drillForm.value, { obtainedMarks: this.totalmarkst, totalMarks: this.updatetotalmarks, status: 1, termId:this.drillForm.value.termId, id: this.drilleditid });
