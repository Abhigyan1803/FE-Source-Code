...
@Component({
  ...
  templateUrl: './add-schedule-of-central-lec.component.html',
  styleUrls: ['./add-schedule-of-central-lec.component.scss']
})
export class AddScheduleOfCentralLecComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private service: TrgTeamService, private snackbar: MatSnackBar, public dialog: MatDialog,
    ...
    if (this.router.url.includes('view-schedule')) {
      ...
      this.service.getlCentralLectureDetails(this.id).subscribe(
        res => {
          if (res.status == '1') {
            console.log(res);
  ...
  addDocument() {
    ...
    if (this.addSopForm.valid) {
      ...
      this.spinner.show();
      this.service.addSCentralLecture(this.addSopForm.value).subscribe(
        res => {
          if (res.status == '1') {
            this.spinner.hide()
  ...
  updateDocument() {
    if (this.addSopForm.valid) {
      this.spinner.show();
      this.service.updatelCentralLectureDoc(this.id, this.addSopForm.value).subscribe(
        res => {
          if (res.status == '1') {
            this.spinner.hide()
            this.openSnackbar(res.msg)
