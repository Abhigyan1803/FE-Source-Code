...
export class ReportComponent implements OnInit {
  ...
  getReport() { 
    this._trgTeam.getReport().subscribe(res => {
      console.log(res);
      if (res.status == "1") {
        this.reportList = res.List;
        this.reportList = this.reportList.map((res) => ({
  ...
  changeStatus(e, l) {
    this.spinner.show();
    if (e.checked) {
      this._trgTeam.changeReportStatus(l.id, 1).subscribe(
        res => {
          if (res.status == '1') {
      ...
      this._trgTeam.changeReportStatus(l.id, 0).subscribe(
        res => {
          if (res.status == '1') {
            this._trgBattalion.openSnackbar(res.msg)
            this.cdref.detectChanges();
  ...
  sortData(sort: Sort) {
    const datalist = this.reportList.slice();
    if (!sort.active || sort.direction === '') {
      this.sortedData = datalist;
      return;
    }
    this.reportList = datalist.sort((a: any, b: any) => {
