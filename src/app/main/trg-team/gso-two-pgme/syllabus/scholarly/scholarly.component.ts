...
@Component({
  selector: 'ms-scholarly',
  templateUrl: './scholarly.component.html',
  styleUrls: ['./scholarly.component.scss']
})
export class ScholarlyComponent implements OnInit {
  ...
  getSyllabusList() {
    this.spinner.show()
    this.service.getSyllabusList('Scholarly', this.termId,2).subscribe(
      res => {
        if (res.status == "OK") {
  ...
  addSyllabus() {
    ...
    this.router.navigate(['/main/admin/trg-team/gso-2-pgme/'+this.term+'/syllabus/scholarly/add-syllabus'])
      } else {
    this.router.navigate(['/main/trg-team/gso-2-pgme/'+this.term+'/syllabus/scholarly/add-syllabus'])
      }
  }

  viewSyllabus(s) {
    if(this.router.url.includes('/main/admin/trg-team/')){
    this.router.navigate(['/main/admin/trg-team/gso-2-pgme/'+this.term+'/syllabus/scholarly/view-syllabus'], { queryParams: { id: s.id } })
        } else {
    this.router.navigate(['/main/trg-team/gso-2-pgme/'+this.term+'/syllabus/scholarly/view-syllabus'], { queryParams: { id: s.id } })
        }
  }
  ...
  openDoc(d) {
    const dialogRef = this.dialog.open(DialogComponent, {
      ...
      data: {
        type: 'document', title:'Syllabus Scholarly', url: d
      }
    });
  }
  ...
  changeStatus(e,l){
    ...
    if(e.checked){
      this.service.updateSyllabusStatus(l.id,1).subscribe(
        res =>{    
          if(res.status == 'OK'){
            this.sharedService.openSnackbar("Status updated successfully")
