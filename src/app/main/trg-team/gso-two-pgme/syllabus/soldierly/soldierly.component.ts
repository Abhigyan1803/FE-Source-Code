...
@Component({
  selector: 'ms-soldierly',
  templateUrl: './soldierly.component.html',
  styleUrls: ['./soldierly.component.scss']
})
export class SoldierlyComponent implements OnInit {
  @ViewChild(MatSort) sort: MatSort;
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  dataSource: any;
  displayedColumns: string[] = ['number', 'date', 'name','description','term', 'status', 'document', 'action'];
  sortedData: any[];
  syllabusList: any[];
  term:string;
  termId:number;
  ...
  getSyllabusList() {
    this.spinner.show()
    this.service.getSyllabusList('Soldierly', this.termId,2).subscribe(
      res => {
        if (res.status == "OK") {
  ...
  addSyllabus() {
    ...
    this.router.navigate(['/main/admin/trg-team/gso-2-pgme/'+this.term+'/syllabus/soldierly/add-syllabus'])
          } else {
            this.router.navigate(['/main/trg-team/gso-2-pgme/'+this.term+'/syllabus/soldierly/add-syllabus'])
          }
  }

  viewSyllabus(s) {
    if(this.router.url.includes('/main/admin/trg-team/')){
      this.router.navigate(['/main/admin/trg-team/gso-2-pgme/'+this.term+'/syllabus/soldierly/view-syllabus'], { queryParams: { id: s.id } })
    } else {
            this.router.navigate(['/main/trg-team/gso-2-pgme/'+this.term+'/syllabus/soldierly/view-syllabus'], { queryParams: { id: s.id } })
          }
  }
  ...
  openDoc(d) {
    const dialogRef = this.dialog.open(DialogComponent, {
      ...
      data: {
        type: 'document', title:'Syllabus Soldierly', url: d
      }
    });
  }
