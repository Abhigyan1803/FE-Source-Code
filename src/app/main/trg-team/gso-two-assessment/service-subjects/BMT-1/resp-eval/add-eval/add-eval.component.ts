...
@Component({
  selector: 'ms-add-eval',
  templateUrl: './add-eval.component.html',
  styleUrls: ['./add-eval.component.scss']
})
export class AddEvalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-eval')) {
      this.title = 'Add Mark'
    }
    else if (this.router.url.includes('view-eval')) {
      this.spinner.show()
  ...
  goBack(){
    this.router.navigate(['/main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt1/resp-eval']); 
  }
}
