...
export class ConfirmationMarkComponent implements OnInit {
  ...
  constructor(private router: Router, private route:ActivatedRoute, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    getPOC(Confirmation){
      this.spinner.show();
    this.adminservice.getConfirmationLIST(Confirmation).subscribe(res =>{  
      console.log(res);
    ...
    addNominal() {
      this.router.navigate(['main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt1/confirmation-mark/add-mark']);
    }
    ...
    view(id) {
      this.router.navigate(['main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt1/confirmation-mark/view-mark'],{queryParams:{id:id}})  
    }
