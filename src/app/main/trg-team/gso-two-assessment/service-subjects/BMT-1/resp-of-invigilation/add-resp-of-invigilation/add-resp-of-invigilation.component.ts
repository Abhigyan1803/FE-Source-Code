...
export class AddRespOfInvigilationComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-invigilations')) {
      this.title = 'Add Mark'
    }
    else if (this.router.url.includes('view-invigilations')) {
      this.spinner.show()
  ...
  goBack(){
    this.router.navigate(['/main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt1/resp-of-invigilation']); 
  }
}
