...
export class AddInstructionComponent implements OnInit {
  ...
  constructor(
    ...
    this.addNominalForm = this.fb.group({
      ...
      studyMaterialType:['GeneralInstruction'],
      doc: []
    })
  }

  ngOnInit(): void {
    if (this.router.url.includes('add-instruction')) {
      this.title = 'Add Mark'
    }
    else if (this.router.url.includes('view-instruction')) {
      this.spinner.show()
  ...
  goBack(){
    this.router.navigate(['/main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt1/general-instruction']); 
  }
}
