...
@Component({
  selector: 'ms-add-retest',
  templateUrl: './add-retest.component.html',
  styleUrls: ['./add-retest.component.scss']
})
export class AddRetestComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.router.url.includes('add-retest')) {
      this.title = 'Add Mark'
    }
    else if (this.router.url.includes('view-retest')) {
      this.spinner.show()
  ...
  goBack(){
    this.router.navigate(['/main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt2/retest']); 
  }
}
