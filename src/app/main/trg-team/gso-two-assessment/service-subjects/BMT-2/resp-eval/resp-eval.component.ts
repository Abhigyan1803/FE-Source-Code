...
@Component({
  selector: 'ms-resp-eval',
  templateUrl: './resp-eval.component.html',
  styleUrls: ['./resp-eval.component.scss']
})
export class RespEvalComponent implements OnInit {
  ...
  constructor(private router: Router, private route:ActivatedRoute, private adminservice: AdminService, private spinner:NgxSpinnerService,
    ...
    getPOC(BMT2Eval){
      this.spinner.show();
    this.adminservice.getEvalLIST(BMT2Eval).subscribe(res =>{  
      console.log(res);
    ...
    addNominal() {
      this.router.navigate(['main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt2/resp-eval/add-eval']);
    }
    ...
    vieweval(id) {
      this.router.navigate(['main/trg-team/gso-2-assessment/'+this.term+'/service-subjects/Bmt2/resp-eval/view-eval'],{queryParams:{id:id}})  
