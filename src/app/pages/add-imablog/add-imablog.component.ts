...
@Component({
  selector: 'ms-add-imablog',
  templateUrl: './add-imablog.component.html',
  styleUrls: ['./add-imablog.component.scss']
})
export class AddImablogComponent implements OnInit {
  ...
  userName = localStorage.getItem('battalian')
  ...
  constructor(private fb: FormBuilder, private router: Router, private spinner: NgxSpinnerService, private authService: AuthService, private dialog: MatDialog
    ...
    if (this.userDetails.battalian != undefined) {
      console.log('111');

      localStorage.setItem('bid', this.userDetails.battalian.id)
      this.bid = this.userDetails.battalian.id
  ...
  goBack() {
    this.router.navigate(['/gc/content/ima-blog'])
  }
