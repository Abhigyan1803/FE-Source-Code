...
export class SyllabusComponent implements OnInit {
  ...
  mainSyllabusChanged(e: any) {
    ...
    if (e.index == '1') {
      ...
      this.subject = 'Military History';
      this.getAcademicSyllabus(this.term_Id, this.paperName, this.subject);
    }
  ...
  getServiceSubSyllabus(type) {
    ...
    this.service.getSyllabusList(type, this.term_Id, 1).subscribe(
      res => {
  ...
  paperChanged(e: any) {
    ...
    if (paper == 'Paper 1') {
      ...
      if (paper == 'Paper 2') {
        ...
        if (paper == 'Paper 3') {
          this.subject = 'Science and Warfare'
        } else
          if (paper == 'Paper 4') {
            this.subject = 'SWT'
          } else
  ...
  acadSyllSubjectChanged(e: any) {
    this.subject = e.tab.textLabel;
    this.getAcademicSyllabus(this.term_Id, this.paperName, this.subject)

  }
  getAcademicSyllabus(termId, paper, subject) {
    Object.assign(this.syllabusData, { type: '', data: '' })
    this.service.getSyllabusListAcademic(termId, paper, subject, 1).subscribe(
      res => {
        // console.log(res);
        if (res.status == 'OK') {

          this.service.setHomePageSyllabus(
            Object.assign(this.syllabusData, { type: 'academic', data: res.object })
          )
